...
achtergrond_fase2 = pygame.image.load(
    r"TESTPYGAM_1._\achtergrond\fase 2 blue sky.jpg"                    
)


achtergrond_phase3 = pygame.image.load(
    r"TESTPYGAM_1._\achtergrond\fase 2 achtergrond.jpg"
)

achtergrond_final = pygame.image.load(
    r"TESTPYGAM_1._\achtergrond\fase 2 final stage.jpg"
)
...
class Bullet:
    ...
    def __init__(self, x, y, width=15, height=15, image=None) -> None:
...
class GalickGun:
    ...
    def should_destroy(self) -> None:
        return pygame.time.get_ticks() - self.spawn_time > self.duration


class WaveBullet:

    def __init__(self, x, y, speed_x, amplitude, frequency) -> None:
...
class Player:
    ...
    def shoot(self, bullets, phase) -> None:
    ...
    def super_attack(self, bullets) -> None:
...
class Boss:
    def __init__(self) -> None:
            ...
            self.phase3 = False  #  nieuwe background fase (tussen 400 en final)
    ...
    def update(self) -> None:
        ...
        # =========================
        if self.phase == 2 and self.hp <= 400 and not self.final_form:
                        
            print("PHASE 3 BACKGROUND")
        ...
        if self.phase == 2 and self.hp <= 50 and not self.final_mode:
    ...
    def attack(self, enemy_bullets, minis, player, rockets) -> None:
    ...
    def take_damage(self, is_super=False) -> None:
...
class Game:
    ...
    def update(self) -> None:
        ...
        if self.boss.phase == 2 and 600 >= self.boss.hp > 400 and not self.spawned_doodles:
        ...
        if self.boss.phase == 2 and 600 >= self.boss.hp > 400:
        ...
        # ================================
        if self.boss.phase == 2 and self.boss.hp <= 200:
    ...
    def draw(self) -> None:
        ...
        if self.boss.phase == 1:
            ...
            if self.boss.hp <= 50:
                ...
                frame.blit(achtergrond_final, (self.bg_x, 0))
                frame.blit(achtergrond_final, (self.bg_x + BREEDTE, 0))

            # 💀 TRANSFORM / MID PHASE (HP ≤ 400)
            elif self.boss.hp <= 400:

                frame.blit(achtergrond_phase3, (self.bg_x, 0))
                frame.blit(achtergrond_phase3, (self.bg_x + BREEDTE, 0))
...
def fade(screen, width, height, speed=5) -> None:
...
def set_controls(value, scheme) -> None:
